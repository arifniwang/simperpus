...
ob_start();
include "../koneksi.php";
$tgldendaawal = date('Y-m-d', strtotime($_GET['tgldendaawal']));
$tgldendaakhir = date('Y-m-d', strtotime($_GET['tgldendaakhir']));
?>
...
<table width="1000" border="0">
  <tr>
    <td width="1000" height="22" align="center"><span style="font-size:18px; font-weight:bold;">DATA DENDA</span></td>
    </tr>
  <tr>
    <td align="Left">&nbsp;</td>
    </tr>
  <tr>
    <td align="Left">Periode : <?php echo date('d-M-Y', strtotime($tgldendaawal)); ?> s/d <?php echo date('d-M-Y', strtotime($tgldendaakhir)); ?></td>
    </tr>
</table><br>
<?php
...
FROM
    ...
    LEFT JOIN `simperpus`.`penerbit` 
        ON (`buku`.`idpenerbit` = `penerbit`.`idpenerbit`) where `peminjaman_detail`.`tgldikembalikan` is not null and `peminjaman_detail`.`tgldikembalikan` between '$tgldendaawal' and '$tgldendaakhir' order by `peminjaman`.`idpinjam` ");
?>
<table width="1000" height="102" border="1" cellpadding="0" cellspacing="0">
  <tr bgcolor="#999999">
    ...
    <td width="68" align="center" bgcolor="#CCCCCC">Denda</td>
    </tr>
<?php 
$tanggungan = 0;
...
{ ?>
  <tr>
    ...
    <?php $tanggungan += $data['denda']; ?>
  </tr>
    <?php
}
  ?>
  <tr>
    <td height="32" colspan="6" align="right">Total Denda</td>
