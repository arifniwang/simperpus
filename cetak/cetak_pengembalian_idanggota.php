...
<table width="1000" border="0">
  <tr>
    <td width="1000" height="22" align="center"><span style="font-size:18px; font-weight:bold;">DATA PENGEMBALIAN BERDASARKAN ANGGOTA</span></td>
    </tr>
  <tr>
    <td align="Left">&nbsp;</td>
  </tr>
...
FROM
    ...
    LEFT JOIN `simperpus`.`penerbit` 
        ON (`buku`.`idpenerbit` = `penerbit`.`idpenerbit`) where `peminjaman_detail`.`tgldikembalikan` is not null and `peminjaman`.`idanggota` like '$idanggota' order by `peminjaman`.`idpinjam` ");
?>
