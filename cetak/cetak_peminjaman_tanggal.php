...
ob_start();
include "../koneksi.php";
$tglpinjamawal = date('Y-m-d', strtotime($_GET['tglpinjamawal']));
$tglpinjamakhir = date('Y-m-d', strtotime($_GET['tglpinjamakhir']));
?>
...
<table width="1000" border="0">
  <tr>
    <td width="1000" height="22" align="center"><span style="font-size:18px; font-weight:bold;">DATA PEMINJAMAN BERDASARKAN TANGGAL PINJAM</span></td>
    </tr>
  <tr>
    <td align="Left">&nbsp;</td>
    </tr>
  <tr>
    <td align="Left">Periode : <?php echo date('d-M-Y', strtotime($tglpinjamawal)); ?> s/d <?php echo date('d-M-Y', strtotime($tglpinjamakhir)); ?></td>
    </tr>
</table><br>
<?php
...
FROM
    ...
    LEFT JOIN `simperpus`.`penerbit` 
        ON (`buku`.`idpenerbit` = `penerbit`.`idpenerbit`) where `peminjaman_detail`.`tgldikembalikan` is null and `peminjaman`.`tglpinjam` between '$tglpinjamawal' and '$tglpinjamakhir' order by `peminjaman`.`idpinjam` ");
?>
<table width="1000" height="102" border="1" cellpadding="0" cellspacing="0">
  <tr bgcolor="#999999">
    ...
    <td width="68" align="center" bgcolor="#CCCCCC">Jumlah Pinjam</td>
    </tr>
<?php 
$tanggungan = 0;
...
{ ?>
  ...
  ?>
  <tr>
    <td height="32" colspan="7" align="right">Total Peminjaman</td>
